...
def create(data: GroupCreate, session: DatabaseSession) -> Group:
    return create_group(session, data.name)
...
def patch(group_id: int, data: GroupUpdate, session: DatabaseSession) -> Group:
    return update_group(session, group_id, data.name)
...
def add_employee(group_id: int, employee_id: int, session: DatabaseSession) -> Employee:
    return add_employee_to_group(session, group_id, employee_id)
...
def remove_employee(group_id: int, employee_id: int, session: DatabaseSession) -> Response:
    remove_employee_from_group(session, group_id, employee_id)
...
def add_policy(group_id: int, policy_id: int, session: DatabaseSession) -> Policy:
    return add_policy_to_group(session, group_id, policy_id)
...
def remove_policy(group_id: int, policy_id: int, session: DatabaseSession) -> Response:
    remove_policy_from_group(session, group_id, policy_id)
    return Response(status_code=status.HTTP_204_NO_CONTENT)
