...
export function SecurityForm({ initialSecurity }: { initialSecurity: AccountSecurity }) {
  ...
  return <div className="detail-grid">
    ...
    <aside className="detail-aside">
      <section className="panel"><div className="panel-header"><div><h2 className="panel-title">Active devices</h2><div className="panel-caption">Sessions expire after inactivity and have a fixed lifetime.</div></div><Laptop size={16} /></div><div className="panel-body">{security.sessions.map((item) => <div className="activity-item" key={item.id}><strong>{item.current ? "This device" : item.user_agent?.split(" ").slice(0, 3).join(" ") || "Unknown device"}</strong><small>{item.last_ip || "Unknown address"} · {new Date(item.last_seen_at).toLocaleString()}</small></div>)}<div className="form-footer"><button className="button secondary" disabled={busy || security.sessions.length < 2} onClick={revokeOthers}>Sign out others</button><button className="button danger" disabled={busy} onClick={revokeAll}>Sign out all</button></div></div></section>
      <section className="panel"><div className="panel-header"><div><h2 className="panel-title">Security events</h2><div className="panel-caption">Review failed sign-ins, new addresses, and account changes.</div></div></div><div className="panel-body">{security.events.length === 0 && <p className="form-hint">No security events recorded.</p>}{security.events.map((item) => <div className="activity-item" key={item.id}><strong>{item.event_type.replaceAll("_", " ")}</strong><small>{new Date(item.created_at).toLocaleString()} · {item.severity}</small>{!item.acknowledged_at && item.severity !== "info" && <button className="button secondary" onClick={() => acknowledge(item.id)}>Acknowledge</button>}</div>)}</div></section>
    </aside>
  </div>;
}
