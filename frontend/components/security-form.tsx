...
type ErrorResult = { error?: { issues?: { message?: string }[]; message?: string }; detail?: string };
...
export function SecurityForm({ initialSecurity }: { initialSecurity: AccountSecurity }) {
  ...
  async function call(path: string, method = "POST", body?: object) {
    ...
    if (!response.ok) throw new Error(message(result, "The security change could not be completed."));
  ...
  async function changePassword(event: FormEvent) {
    ...
    try {
      await call("/auth/change-password", "POST", { current_password: currentPassword, new_password: newPassword });
      setCurrentPassword(""); setNewPassword(""); setConfirmation("");
      setNotice("Password changed. Other active sessions were signed out."); router.refresh();
  ...
  async function disableMfa(event: FormEvent) {
    ...
    try {
      ...
      setSecurity((value) => ({ ...value, mfa_enabled: false })); setMfaPassword(""); setMfaCode("");
      setNotice("MFA was disabled and all previous sessions were revoked."); router.refresh();
  ...
  async function stepUp(event: FormEvent) {
    event.preventDefault(); setBusy(true); setError("");
    try {
      await call("/auth/reauthenticate", "POST", { password: stepPassword, ...(stepCode ? { mfa_code: stepCode } : {}) });
      setStepPassword(""); setStepCode(""); setNotice("Identity verified. Sensitive actions are unlocked for 10 minutes."); router.refresh();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Identity verification failed."); }
    finally { setBusy(false); }
  }
  ...
  async function revokeOthers() {
    ...
    try {
      ...
      setSecurity((value) => ({ ...value, sessions: value.sessions.filter((item) => item.current) }));
      setNotice("Every other device has been signed out.");
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Sessions could not be revoked."); }
    finally { setBusy(false); }
  }

  async function revokeAll() {
    setBusy(true); setError("");
    try { await call("/auth/sessions/revoke-all"); } finally { router.replace("/login"); router.refresh(); }
  }

  async function acknowledge(id: number) {
    try {
      const result = await call(`/auth/security-events/${id}/acknowledge`);
      setSecurity((value) => ({ ...value, events: value.events.map((item) => item.id === id ? result as typeof item : item) }));
    } catch (reason) { setError(reason instanceof Error ? reason.message : "The event could not be acknowledged."); }
  }

  return <div className="detail-grid">
    <div className="detail-main">
      {error && <div className="error-banner" role="alert"><CircleAlert size={14} />{error}</div>}
      ...
      <section className="panel security-panel"><div className="panel-header"><div><h2 className="panel-title">Confirm your identity</h2><div className="panel-caption">Sensitive administrative actions require a recent password and MFA check.</div></div></div><div className="panel-body"><form className="security-form" onSubmit={stepUp}><label className="field"><span className="field-label">Password</span><input className="input" required type="password" value={stepPassword} onChange={(event) => setStepPassword(event.target.value)} /></label>{security.mfa_enabled && <label className="field"><span className="field-label">Authenticator code</span><input className="input" required inputMode="numeric" value={stepCode} onChange={(event) => setStepCode(event.target.value)} /></label>}<button className="button" disabled={busy} type="submit">Verify identity</button></form></div></section>
    </div>
    <aside className="detail-aside">
      <section className="panel"><div className="panel-header"><div><h2 className="panel-title">Active devices</h2><div className="panel-caption">Sessions expire after inactivity and have a fixed lifetime.</div></div><Laptop size={16} /></div><div className="panel-body"><div className="security-list">{security.sessions.map((item) => <div className="security-list-item" key={item.id}><strong>{item.current ? "This device" : item.user_agent?.split(" ").slice(0, 3).join(" ") || "Unknown device"}</strong><small>{item.last_ip || "Unknown address"} · {new Date(item.last_seen_at).toLocaleString()}</small></div>)}</div><div className="security-actions"><button className="button secondary" disabled={busy || security.sessions.length < 2} onClick={revokeOthers}>Sign out others</button><button className="button danger" disabled={busy} onClick={revokeAll}>Sign out all</button></div></div></section>
      <section className="panel"><div className="panel-header"><div><h2 className="panel-title">Security events</h2><div className="panel-caption">Review failed sign-ins, new addresses, and account changes.</div></div></div><div className="panel-body">{security.events.length === 0 && <p className="form-hint">No security events recorded.</p>}<div className="security-list">{security.events.map((item) => <div className="security-list-item" key={item.id}><strong>{item.event_type.replaceAll("_", " ")}</strong><small>{new Date(item.created_at).toLocaleString()} · {item.severity}</small>{!item.acknowledged_at && item.severity !== "info" && <button className="button secondary small" onClick={() => acknowledge(item.id)}>Acknowledge</button>}</div>)}</div></div></section>
    </aside>
  </div>;
}
