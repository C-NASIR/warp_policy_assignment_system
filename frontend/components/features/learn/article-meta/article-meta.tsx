...
export function ArticleMeta({ page }: { page: LearnPage }) {
  ...
  return (
    <div className="learn-article-meta" aria-label="Article information">
      <span>
        <Clock3 size={13} /> {page.data.reading_time} min read
      </span>
      <span>
        <Users size={13} /> {page.data.audiences.join(", ")}
      </span>
      <span>
        <ShieldCheck size={13} /> {page.data.owner}
      </span>
      <span>
        <CalendarCheck2 size={13} /> Verified {formatDate(page.data.last_verified)}
      </span>
    </div>
  );
}
