...
export function AppShell({ children, connected, currentUser, securityEvents, learnSearchEntries }: { children: React.ReactNode; connected: boolean; currentUser: CurrentUser | null; securityEvents: SecurityEvent[]; learnSearchEntries: LearnSearchEntry[] }) {
  ...
  function runCommand(href: string) {
    setCommandOpen(false); setQuery(""); setActiveIndex(0);
  ...
  return (
    <div className="app-layout">
      ...
      <aside className={`sidebar${menuOpen ? " open" : ""}`} aria-label="Workspace navigation">
        ...
        <nav className="nav-list" aria-label="Primary navigation">{visibleNavigation.map((item) => { const Icon = item.icon; const opensLearn = item.href.startsWith("/learn"); return <Link className={`nav-item${isActive(item.href) ? " active" : ""}`} href={item.href} key={item.href} onClick={() => setMenuOpen(false)} aria-current={isActive(item.href) ? "page" : undefined} target={opensLearn ? "_blank" : undefined} rel={opensLearn ? "noopener noreferrer" : undefined}><Icon size={16} strokeWidth={1.8} />{item.label}{opensLearn && <span className="sr-only"> (opens in a new tab)</span>}</Link>; })}</nav>
