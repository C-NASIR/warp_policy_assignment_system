...
export function PolicyLifecycle({ policy, apiConfigured }: { policy: Policy; apiConfigured: boolean }) {
  ...
  async function updateStatus() {
    ...
    try {
      if (apiConfigured) {
        const response = await fetch(`/api/backend/policies/${policy.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: nextStatus }) });
