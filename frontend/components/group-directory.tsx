...
export function GroupDirectory({ initialGroups, apiConfigured }: { initialGroups: GroupRow[]; apiConfigured: boolean }) {
  ...
  const duplicate = useMemo(() => groups.some((group) => group.name.toLowerCase() === normalized.toLowerCase()), [groups, normalized]);
  ...
  async function createGroup() {
    ...
    if (!apiConfigured) {
      const group = { id: Math.max(...groups.map((item) => item.id), 0) + 1, name: normalized, memberCount: 0, policyCount: 0 };
      setGroups((current) => [...current, group]); setName(""); setCreating(false); setSaving(false);
      return;
    }
    try {
      const response = await fetch("/api/backend/groups", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: normalized }) });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.error?.message ?? result.detail ?? "The group could not be created.");
      router.push(`/groups/${result.id}`); router.refresh();
