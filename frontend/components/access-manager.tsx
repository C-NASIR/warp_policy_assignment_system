    ...
    <>
      <div className="page-heading">
        ...
        {canManage && (
          <button
            className="button"
            onClick={() => (tab === "users" ? setEditingUser(null) : setEditingRole(null))}
          >
...
function UsersPanel({
  ...
  return (
    <div className="data-panel access-table">
      ...
      {roles.map((role) => (
        <article className="panel role-card" key={role.id}>
          ...
          <div className="permission-chip-list">
            <span className="badge accent">{employeeScopeLabels[role.employee_scope]}</span>
            ...
            {role.permissions.length > 5 && (
              <span className="badge accent">+{role.permissions.length - 5}</span>
            )}
          </div>
        </article>
      ))}
