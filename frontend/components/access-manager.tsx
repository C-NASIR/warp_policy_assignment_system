...
import type { Employee, Permission, Role, User } from "@/lib/types";
...
export function AccessManager({ initialUsers, initialRoles, employees, permissions, canManage, canReadEmployees, apiConfigured }: Props) {
...
function RoleForm({ role, permissions, apiConfigured, onClose, onSaved, onError }: { role: Role | null; permissions: Permission[]; apiConfigured: boolean; onClose(): void; onSaved(role: Role): void; onError(text: string): void }) {
  ...
  async function submit(event: FormEvent) {
    ...
    const response = await fetch(`/api/backend/roles${role ? `/${role.id}` : ""}`, { method: role ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, description, permissions: selected, employee_scope: employeeScope }) });
    setBusy(false);
    if (!response.ok) return onError(await errorMessage(response));
