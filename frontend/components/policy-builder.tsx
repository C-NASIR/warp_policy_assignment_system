...
export function PolicyBuilder({ conditionFields, assignmentFields, employees, apiConfigured, basePolicy }: { conditionFields: ConditionField[]; assignmentFields: AssignmentField[]; employees: Employee[]; apiConfigured: boolean; basePolicy?: Policy | null }) {
  ...
  const [outputs, setOutputs] = useState<BuilderOutput[]>(() => (baseVersion?.values.length ? baseVersion.values : [{ assignment_field_definition_id: assignmentFields[0]?.id ?? 1, value: "" }]).map((item, index) => ({ ...item, rowId: index + 1 })));
  ...
  function review() {
    if (!name.trim() || [...conditions, ...childConditions].some((item) => !item.field || !item.value.trim()) || outputs.some((item) => !item.assignment_field_definition_id || !item.value.trim())) {
      setError("Add a policy name and complete every rule and assignment value before previewing impact."); return;
    }
  ...
  async function save() {
    ...
    try {
      const endpoint = basePolicy ? `/api/backend/policies/${basePolicy.id}/versions` : "/api/backend/policies";
      const response = await fetch(endpoint, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(basePolicy ? version : { name: name.trim(), status: "active", ...version }) });
  ...
  return <div className="builder-shell">
    <div className="form-panel">
      ...
      <section className="form-section"><div className="form-section-title">What gets assigned</div><div className="form-section-description">A policy can provide values across several one- or many-value assignment fields.</div><div className="output-list">{outputs.map((output) => { const field = assignmentFields.find((item) => item.id === Number(output.assignment_field_definition_id)); return <div className="output-row" key={output.rowId}>
        ...
        <button className="remove-button" onClick={() => { if (outputs.length > 1) setOutputs((current) => current.filter((item) => item.rowId !== output.rowId)); }} aria-label="Remove assignment"><Trash2 size={13} /></button>
      ...
      <div className="form-footer"><span className="form-hint">Preview the affected population before this version is saved.</span><div className="heading-actions"><button className="button secondary" onClick={review}><Eye size={14} /> Preview impact</button>{previewed && <button className="button" onClick={save} disabled={saving}><Check size={14} />{saving ? "Saving…" : basePolicy ? "Create version" : "Create policy"}</button>}</div></div>
    </div>
    <aside className="form-panel preview-panel">{!previewed ? <div className="preview-empty"><div className="preview-empty-icon"><Users size={19} /></div><h3>Population impact</h3><p>Your current draft matches {matchedEmployees.length} employees. Preview to review the exact population and outputs.</p></div> : <><div className="impact-hero"><Users size={17} /><div className="impact-number">{matchedEmployees.length}</div><div className="impact-label">employees match this policy version</div></div><div className="preview-content"><div className="label">Sample employees</div><div className="match-list">{matchedEmployees.slice(0, 5).map((employee) => <div className="match-person" key={employee.id}><span className="person-cell"><span className="avatar">{initials(employee.name)}</span><span><span className="primary-cell">{employee.name}</span><span className="secondary-cell">{employee.department} · {employee.state}</span></span></span><span className="badge success">Matches</span></div>)}{matchedEmployees.length === 0 && <div className="empty-state" style={{ padding: 18 }}>No employees currently match.</div>}</div>{matchedEmployees.length > 5 && <div className="results-count" style={{ marginTop: 9 }}>+ {matchedEmployees.length - 5} more matching employees</div>}<div className="label" style={{ marginTop: 19 }}>Assignments per match</div>{outputs.map((item) => <div className="preview-assignment" key={item.rowId}><div><div className="preview-field">{assignmentFields.find((field) => field.id === Number(item.assignment_field_definition_id))?.name}</div><div className="preview-value">{item.value}</div></div><span className="preview-change added">+ Assign</span></div>)}</div></>}</aside>
  </div>;
}
