...
export function PolicyBuilder({ conditionFields, assignmentFields, employees, apiConfigured, basePolicy }: { conditionFields: ConditionField[]; assignmentFields: AssignmentField[]; employees: Employee[]; apiConfigured: boolean; basePolicy?: Policy | null }) {
  ...
  async function review() {
    ...
    setError(""); setApproval(null); setEngineAffected(null);
    if (apiConfigured && basePolicy) {
      ...
      try {
        ...
        setApproval(result.approval?.token ?? null); setEngineAffected(result.affected_employee_count ?? 0);
  ...
  return <div className="builder-shell">
    <div className="form-panel">
      ...
      <div className="form-footer"><span className="form-hint">{apiConfigured && basePolicy ? "The engine signs this preview so the exact reviewed change can be executed once." : "Preview the affected population before this version is saved."}</span><div className="heading-actions"><button className="button secondary" onClick={review} disabled={reviewing}><Eye size={14} />{reviewing ? "Calculating…" : "Preview impact"}</button>{previewed && <button className="button" onClick={save} disabled={saving}><Check size={14} />{saving ? "Saving…" : basePolicy ? "Create version" : "Create policy"}</button>}</div></div>
    </div>
