...
export function PolicyBuilder({
    ...
    () =>
      employees.filter((employee) => {
        ...
        if (childConditions.length) {
          const childResults = childConditions.map((condition) =>
            matches(employee, condition, employees),
          );
          results.push(
            childLogic === "and" ? childResults.every(Boolean) : childResults.some(Boolean),
          );
        }
  ...
  async function save() {
    ...
    if (!apiConfigured) {
      setSuccess(
        basePolicy
          ? "New policy version created in demo mode."
          : "Policy created in demo mode with the impact shown.",
      );
      setSaving(false);
      return;
    }
    try {
      ...
      router.push(`/policies/${basePolicy?.id ?? result.id}`);
      router.refresh();
    } catch (reason) {
        ...
        <section className="form-section">
          ...
          <div className="rule-group">
            <div className="rule-group-head">
              ...
              <span className="badge accent">{matchedEmployees.length} match</span>
            </div>
      ...
      <aside className="form-panel preview-panel">
        {!previewed ? (
          <div className="preview-empty">
            ...
            <p>
              Your current draft matches {matchedEmployees.length} employees. Preview to review the
              exact population and outputs.
            </p>
          </div>
        ) : (
          <>
            <div className="impact-hero">
              <Users size={17} />
              <div className="impact-number">{engineAffected ?? matchedEmployees.length}</div>
              <div className="impact-label">
                {engineAffected === null
                  ? "employees match this policy version"
                  : "employees will have resolved assignment changes"}
              </div>
            </div>
            <div className="preview-content">
              <div className="label">Sample employees</div>
              <div className="match-list">
                {matchedEmployees.slice(0, 5).map((employee) => (
                  <div className="match-person" key={employee.id}>
                ...
                {matchedEmployees.length === 0 && (
                  <div className="empty-state" style={{ padding: 18 }}>
                    No employees currently match.
                  </div>
                )}
              </div>
              {matchedEmployees.length > 5 && (
                <div className="results-count" style={{ marginTop: 9 }}>
                  + {matchedEmployees.length - 5} more matching employees
                </div>
              )}
...
function matches(employee: Employee, condition: BuilderCondition, employees: Employee[]) {
  ...
  if (condition.field === "tenure")
    actual = new Date().getUTCFullYear() - new Date(employee.start_date).getUTCFullYear();
  else if (condition.field === "is_manager")
    actual = employees.some((item) => item.manager_id === employee.id);
  else if (condition.field === "direct_report_count")
    actual = employees.filter((item) => item.manager_id === employee.id).length;
  ...
  const expectedRaw = condition.value.startsWith("P")
    ? (condition.value.match(/\d+/)?.[0] ?? "0")
    : condition.value;
