...
import { apiConfigured, getAssignmentFields, getConditionFields, getCurrentUser, getEmployees, getPolicy } from "@/lib/backend";
...
export default async function NewPolicyPage({ searchParams }: PageProps<"/policies/new">) {
  ...
  return <><div className="breadcrumb"><Link href="/policies">Policies</Link><ChevronRight size={11} /><span>{basePolicy ? `New ${basePolicy.name} version` : "Create policy"}</span></div><div className="page-heading"><div><p className="eyebrow">Policy authoring</p><h1>{basePolicy ? "Create a new version" : "Create a policy"}</h1><p className="page-subtitle">Define the population, employee assignments, priority, and effective dates. Review the impact before anything changes.</p></div><span className="badge accent">Draft</span></div><PolicyBuilder conditionFields={conditionFields} assignmentFields={assignmentFields} employees={employees} apiConfigured={apiConfigured} basePolicy={basePolicy} activateOnCreate={activateOnCreate} /></>;
}
