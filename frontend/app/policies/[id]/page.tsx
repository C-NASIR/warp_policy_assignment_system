...
import { apiConfigured, getAssignmentFields, getCurrentUser, getPolicy, getPolicyImpact } from "@/lib/backend";
...
export default async function PolicyDetailPage({ params }: PageProps<"/policies/[id]">) {
  ...
  const [policy, impact, fields] = await Promise.all([getPolicy(Number(id)), getPolicyImpact(Number(id)), !apiConfigured || hasPermission(user, "settings:read") ? getAssignmentFields() : []]);
  if (!policy) notFound();
  ...
  return <><Link className="page-back-link" href="/policies"><ArrowLeft size={13} />Back to policies</Link>
    ...
    <div className="detail-grid"><div className="section-stack">
      ...
      <section className="panel"><div className="panel-header"><h2 className="panel-title">Assignments provided</h2><span className="badge">{assignmentValueCount} {assignmentValueCount === 1 ? "value" : "values"}</span></div><div className="panel-body"><div className="assignment-list">{current?.values.map((value, index) => <div className="assignment-card" key={`${value.assignment_field_definition_id}-${value.value}-${index}`}><div className="assignment-summary policy-assignment-summary"><span><span className="assignment-field">{fieldNames[value.assignment_field_definition_id] ?? `Field ${value.assignment_field_definition_id}`}</span><span className="assignment-value">{value.value}</span></span><span className="assignment-source"><strong>This policy</strong>Priority {current.priority}</span><span className="policy-assignment-status" aria-label="Provided by this policy"><CircleCheckBig size={15} aria-hidden="true" /></span></div></div>)}</div></div></section>
