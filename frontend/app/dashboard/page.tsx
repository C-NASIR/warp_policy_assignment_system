...
export default async function OverviewPage() {
  ...
  const coverage = summary.fields.slice(0, 4).map((item) => ({
    name: item.assignment_field_definition.name,
    caption: `${item.assigned_employee_count} of ${summary.employee_count} employees`,
  ...
  const metrics = [
    {
      ...
      delta: `${summary.employees_with_assignments} covered`,
      icon: Users,
    },
    {
    ...
    },
    {
      ...
      delta: `${summary.employee_count ? Math.round((summary.employees_with_assignments / summary.employee_count) * 100) : 0}% covered`,
      icon: CircleCheckBig,
    },
    {
      label: "Manual overrides",
      value: summary.override_assignment_count.toLocaleString(),
      delta: `${summary.policy_assignment_count.toLocaleString()} policy-derived`,
      icon: ShieldCheck,
    },
  ];
  return (
    <>
        ...
        <Panel as="article">
          <div className="panel-header">
            ...
            <Link className="panel-link" href="/employees">
              View employees <ArrowRight size={13} />
            </Link>
          </div>
          <div className="coverage-list">
            {coverage.map((item) => (
              <div className="coverage-row" key={item.name}>
                ...
                <div className="progress-track" aria-label={`${item.value}% covered`}>
      ...
      <section className="quick-actions" aria-label="Quick actions">
        <Link className="quick-action" href="/employees/new">
          ...
          <span>
            <span className="quick-action-title">Onboard an employee</span>
            <span className="quick-action-caption">Preview assignments before saving</span>
          </span>
        </Link>
        <Link className="quick-action" href="/policies/new">
          <span className="quick-action-icon">
            <BookOpenCheck size={16} />
          </span>
          <span>
            <span className="quick-action-title">Define a policy</span>
            <span className="quick-action-caption">Build rules with guided conditions</span>
          </span>
        </Link>
        <Link className="quick-action" href="/groups">
          ...
          <span>
            <span className="quick-action-title">Review a group</span>
            <span className="quick-action-caption">See inherited policy coverage</span>
          </span>
        </Link>
      </section>
    </>
