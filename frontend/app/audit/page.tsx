...
function auditEntityLabel(
  ...
  if (event.entity_type === "Employee")
    return (
      employees.find((item) => item.id === event.entity_id)?.name ??
      String(snapshot.name ?? "Employee")
    );
  if (event.entity_type === "Policy")
    return (
      policies.find((item) => item.id === event.entity_id)?.name ??
      String(snapshot.name ?? "Policy")
    );
  if (event.entity_type === "Group")
    return (
      groups.find((item) => item.id === event.entity_id)?.name ?? String(snapshot.name ?? "Group")
    );
  ...
  if (event.entity_type === "EmployeeAssignment")
    return employee ? `${employee.name} · Assignment` : "Employee assignment";
  if (event.entity_type === "EmployeeOverride")
    return employee ? `${employee.name} · Manual override` : "Manual override";
  if (event.entity_type === "EmployeeGroupMembership")
    return employee ? `${employee.name} · Group membership` : "Group membership";
