...
export const metadata: Metadata = {
  ...
  description: "Turn employee information into clear, consistent policy assignments. Define rules, preview changes, and understand every decision with PolicyOS.",
};

const features = [
  { icon: GitBranch, title: "Rules that do the repeat work.", copy: "Build policies around employee attributes and group membership. Keep assignments consistent as your organization changes." },
  { icon: SlidersHorizontal, title: "See the impact before you act.", copy: "Preview who a change affects, resolve competing priorities, and route sensitive changes through approval." },
  { icon: History, title: "An answer behind every assignment.", copy: "Trace assignments to their source. Review policy history, manual overrides, and recorded changes in one place." },
];
    ...
    <main id="main">
      <section className={styles.hero} aria-labelledby="hero-title">
        <div className={styles.heroCopy}>
          <p className={styles.eyebrow}><span /> POLICY ASSIGNMENTS, SIMPLIFIED</p>
          <h1 id="hero-title">Every policy.<br />The <span>right people.</span></h1>
          <p className={styles.heroDescription}>Your people change. Their policies should keep up. Turn employee information into consistent assignments—with a clear reason behind every decision.</p>
          <div className={styles.heroActions}><Link className={styles.primaryButton} href="/signup">Sign up for PolicyOS <ArrowRight size={18} /></Link><Link className={styles.secondaryButton} href="/login">Sign in <ArrowRight size={17} /></Link></div>
          <div className={styles.heroNotes}><span><Check size={15} /> Preview before applying</span><span><Check size={15} /> Keep a clear audit trail</span></div>
        </div>
        <div className={styles.visual} aria-label="Illustrative policy assignment workflow">
          <div className={styles.visualHeading}><span><span className={styles.liveDot} /> THE ASSIGNMENT ENGINE</span><span>Example workflow</span></div>
          <div className={styles.personCard}><div className={styles.avatar}>JL</div><div><strong>Jordan Lee</strong><span>Engineering · Full-time</span></div><span className={styles.personLabel}>Employee</span></div>
          <div className={styles.connector}><span /><ArrowDown size={17} /></div>
          <div className={styles.engineCard}><div className={styles.engineTitle}><span className={styles.engineIcon}><Layers3 size={22} /></span><div><strong>Right rules. Resolved.</strong><span>Employee attributes → policy matches</span></div></div><div className={styles.conditions}><span>Department = Engineering</span><span>Employment = Full-time</span></div></div>
          <div className={styles.connector}><span /><ArrowDown size={17} /></div>
          <div className={styles.assignmentCard}><div className={styles.assignmentHeader}><strong>Policy assignments</strong><span><CheckCheck size={15} /> Resolved</span></div>{["Engineering application access", "Full-time benefits", "Standard pay schedule"].map((name) => <div className={styles.assignmentRow} key={name}><span className={styles.checkIcon}><Check size={13} /></span>{name}<span>Policy-derived</span></div>)}</div>
          <div className={styles.explanation}><ShieldCheck size={16} /><span>Every assignment has a reason. Every change leaves a trail.</span></div>
        </div>
      </section>
