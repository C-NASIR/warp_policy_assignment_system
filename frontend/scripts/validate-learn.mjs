...
if (!Array.isArray(ownerRegistry.owners)) throw new Error("content-owners.yml must define an owners list");
for (const owner of ownerRegistry.owners) {
  if (typeof owner.name !== "string" || typeof owner.area !== "string" || !["quarterly", "semiannual"].includes(owner.cadence)) {
    failures.push(`${relative(ownerRegistryFile)}: every owner needs a name, area, and valid cadence`);
  }
}
const owners = new Set(ownerRegistry.owners.map((owner) => owner.name));
if (owners.size !== ownerRegistry.owners.length) failures.push(`${relative(ownerRegistryFile)}: owner names must be unique`);
...
for (const article of articles) {
  ...
  for (const evidence of data.verified_by ?? []) {
    ...
    else if (!fs.existsSync(path.join(repoRoot, evidence))) fail(file, `verified_by target does not exist: ${evidence}`);
  }
  if (ids.has(data.content_id)) fail(file, `duplicate content_id ${data.content_id} (also ${relative(ids.get(data.content_id))})`);
  ...
  if (Number.isNaN(lastVerified.valueOf()) || Number.isNaN(reviewBy.valueOf())) fail(file, "review dates must use YYYY-MM-DD");
  else {
    ...
    if (data.review_by < new Date().toISOString().slice(0, 10)) fail(file, `content review is overdue (${data.review_by})`);
  }
  if (!["approved", "retired"].includes(data.status)) warnings.push(`${data.content_id} remains ${data.status} (${data.owner})`);

  const rel = path.relative(contentRoot, file).replace(/\.mdx$/, "").split(path.sep);
  const route = rel.join("/") === "index" ? "/learn" : `/learn/${rel.join("/")}`;
  routes.add(route);
  article.route = route;
  article.links = [...body.matchAll(/(?:\]\(|href=["'])(\/learn(?:\/[a-z0-9-]+){0,2})(?:#[^)'"\s]+)?/g)].map((match) => match[1].replace(/\/$/, ""));
}
...
for (const section of sections) {
  ...
  let pages;
  try { pages = JSON.parse(fs.readFileSync(metaFile, "utf8")).pages; }
  catch (error) { fail(metaFile, `invalid metadata: ${error.message}`); continue; }
  const expected = articles.filter((article) => article.data.section === section && path.basename(article.file) !== "index.mdx").map((article) => path.basename(article.file, ".mdx"));
  for (const slug of expected) if (pages.filter((item) => item === slug).length !== 1) fail(metaFile, `${slug} must appear exactly once`);
  for (const slug of pages) if (!expected.includes(slug)) fail(metaFile, `unknown or misclassified page ${slug}`);
}

const rootMetaFile = path.join(contentRoot, "meta.json");
const rootPages = JSON.parse(fs.readFileSync(rootMetaFile, "utf8")).pages;
for (const requiredPage of ["index", ...sections]) if (rootPages.filter((item) => item === requiredPage).length !== 1) fail(rootMetaFile, `${requiredPage} must appear exactly once`);

const sourceFiles = walk(path.join(frontendRoot, "app"), ".tsx").concat(walk(path.join(frontendRoot, "components"), ".tsx"));
...
if (failures.length) {
  console.error(`Learn quality gate failed with ${failures.length} problem(s):\n- ${failures.join("\n- ")}`);
  process.exit(1);
}

const ownerCounts = new Map();
for (const { data } of articles) ownerCounts.set(data.owner, (ownerCounts.get(data.owner) ?? 0) + 1);
console.log(`Learn quality gate passed: ${articles.length} articles, ${routes.size} routes, ${owners.size} registered owners.`);
console.log(`Ownership: ${[...ownerCounts].sort().map(([owner, count]) => `${owner} (${count})`).join(", ")}`);
if (warnings.length) console.log(`Review queue: ${warnings.length} article(s) await organizational approval.`);
