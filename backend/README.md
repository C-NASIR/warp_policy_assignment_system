...
Every business API endpoint except `GET /` requires either a valid human session or `Authorization: Bearer <credential>`. The frontend creates the one-time Root account through `POST /auth/setup-root`, then uses login sessions stored in an HTTP-only, SameSite `strict` cookie. Root is an immutable break-glass superuser; other users receive the union of their assigned role permissions and employee-data scopes. Scope is enforced on employee records, assignments, overrides, group membership, summaries, policy impact, change previews and executions, employee-linked user data, and employee-related audit events. Out-of-scope direct employee requests return 404 so they do not disclose whether the record exists. Permission and scope changes take effect on the next request, including for existing sessions. `AUTH_SESSION_TTL_SECONDS` defaults to 12 hours, and `AUTH_SESSION_COOKIE_SECURE` must be `true` behind production HTTPS. Passwords use Argon2id; only session-token hashes are stored in PostgreSQL. State-changing session requests also require a trusted `Origin`.
...
Phase 3 includes role-scoped employee visibility, reporting-tree traversal, user-to-employee links, filtered employee-derived results, hidden direct lookups, and scope-aware mutation previews. Root and machine credentials retain global employee access. Older API clients that omit `employee_scope` when creating a role retain the pre-Phase-3 `all` behavior; the first-party UI makes the choice explicit and defaults new roles to `none`. Existing roles are similarly migrated to `all` during startup so the upgrade does not silently remove access. It intentionally excludes password-reset delivery, MFA, external identity-provider integration, attribute-based scopes beyond the reporting hierarchy, and delegated role-administration boundaries. The domain engine also still excludes retroactive assignment-history rewriting, scheduled future employee facts and relationship changes, time-bounded overrides, override reasons and authorship, administrator-defined condition formulas, an internal worker timer or deployment scheduler, queues, and caching. API credentials remain intended for the MCP server and automation rather than browser users.
